...
options:
  ...
  endpoint_type:
    description:
      - Specify what type of endpoint to create. If you are creating an S3 or
        DynamoDB endpoint, these are 'Gateway' type endpoints. All others are
        'Interface' type endpoints.
    required: true
    default: Interface
  security_group_ids:
    description:
      - A list of VPC security group id's to apply to the endpoint. Can only
        be used with interface type endpoints
    required: false
  wait:
    description:
  ...
  route_table_ids:
    description:
      - List of one or more route table ids to attach to the endpoint. A route
        ...
        provided. Do not provide route tables if creating an interface type
        endpoint.
    required: false
  vpc_endpoint_id:
    description:
...
def create_vpc_endpoint(client, module):
    ...
    if module.params.get('endpoint_type'):
        params['VpcEndpointType'] = module.params.get('endpoint_type')
    else:
